...
# Read expressions
exp = pd.read_csv('mRNA_count_155pair.tsv', sep='\t', index_col=0)  # log2+1 transformed; Genes in columns

# Calc signature scores
signature_scores = ssgsea_formula(exp.T, gmt)

# Scale signatures
signature_scores_scaled = median_scale(signature_scores)
# signature_scores_scaled = pd.read_csv('signatures-panmi.tsv',sep='\t',index_col=0)
...
final_clusters = detect_type(clustering_metrics.T[best_threshold].perc, signature_scores_scaled)
